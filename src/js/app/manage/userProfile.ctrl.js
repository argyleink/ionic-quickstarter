  ...
  var UserProfileCtrl = /*@ngInject*/function ($scope, $rootScope, $state, $timeout, $ionicModal, $ionicActionSheet, $translate, $q, $stateParams, Application, ImageService, FileManager, UserService) {
    ...
    var ImageUpload = function () {
      ...
      this.setUploaded = function (newImage) {
        // if a new file was uploaded successfully then remove the old/previous file, if any (each time a new image is uploaded we generate a new unique file name, this is for HTML "cache busting")
    ...
    function getImageOpts() {
      ...
      // set image size to 600 pixels, larger is useless; otherwise the JPEG file will be huge
      return {
        pictureQuality: 50,
        targetSize:     600
      }
    ...
    function addImage(type, saveImageFn, cropImageFn, fileName) {
      ...
      var targetDir = cordova.file.dataDirectory
      // file URL on the native file system
      var fileUrl = null

      // Now execute all the steps of the "pipeline" via Promise chaining:
      ImageService.getPicture(type, imageOpts.pictureQuality, imageOpts.targetSize)
    ...
    $ionicModal.fromTemplateUrl('js/app/manage/image-crop-modal.html', {
      scope:      $scope,
    ...
    vm.showImageCropModal = function(image, widthHeightRatio) {
      vm.imageCropModal.show()
      .then(function () {
        // NOTE: for cropping to work, the source (original) and target (cropped) image variables should be put in a container object (see below, $scope.image), NOT directly in the $scope variable itself; for background see:
        // https://github.com/alexk111/ngImgCrop/issues/18#issuecomment-78911464

        // add an object to $scope which wraps the to-be-cropped image and the cropped (result) image; otherwise it will not work (see https://github.com/alexk111/ngImgCrop/issues/18). We also add config properties to it.
        $scope.image = {
          originalImage:  image,
          croppedImage:   '',
    ...
    function gotoNextPage() {
      ...
      if (message) Application.showToast(message)

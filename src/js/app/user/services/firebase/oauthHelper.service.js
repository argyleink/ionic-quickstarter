...
angular.module('app.oauthUtil', ["oauth.utils"])
  .factory("oauthHelper", ["$q", '$http', "$cordovaOauthUtility", function($q, $http, $cordovaOauthUtility) {
    ...
    return {
      /*
        Initialize an auth against the Twitter service, but do not complete it (use a hidden InAppBrowser winndow, and close the window after a few seconds and do not complete the request).
       
        The goal is to mimick a "logout" call (which the Twitter API does not officially support) by passing the URL params "force_login=true" and "screen_name=" (empty Twitter user name).
          ...
          if ($cordovaOauthUtility.isInAppBrowserInstalled(cordovaMetadata) === true) {
            ...
            if (typeof jsSHA !== "undefined") {
              ...
              .then(function (response) {
                ...
                // Pass the URL params "screen_name=" and "force_login=true" to force re-authentication, but don't complete the auth flow (the Twitter login form is displayed in a hidden Inappbrowser window), thus in effect performing a "logout"
                var url = 'https://api.twitter.com/oauth/authenticate?screen_name=&force_login=true&oauth_token=' + parameterMap.oauth_token

                // set "hidden=yes" because we don't want to show the login form to the user (we're not interested in performing an actual login)

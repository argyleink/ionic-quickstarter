...
appModule('app.user')
  .factory('UserServiceFirebaseImpl', function ($q, $log, $rootScope, Stopwatch, AppHooks, User, fbutil, $firebaseAuth, $firebaseObject, $firebaseArray, oauthHelper, TwitterReauthentication) {
    ...
    function setCurrentUser(userData) {
      ...
      // we have a logged in user, load their data
    ...
    var changePassword = function (email, passwordOld, passwordNew) {
      ...
      auth.$changePassword({email: email, oldPassword: passwordOld, newPassword: passwordNew})
      .then(function() {
    ...
    var loginWithTwitter = function() {
      ...
      .then(function(authData) {
        ...
        // create/update user profile in our data store; for simplicity we perform an update every time the user signs in via Twitter; this way, for Twitter login, we don't have to distinguish between "signup" (initial, first time) and "login" (recurring) - login and signup are (for Twitter) one and the same thing - on every login the user's profile is updated
    ...
    var logoutApp = function () {
      ...
      if (provider) return provider.logout()
    ...
    var logoutTwitter = function() {
      // Reauthentication hack: the Twitter API does not really offer a "logout" function, so what we do is we simply start an auth attempt with "force_login=true" (see code of the "oauthHelper" service)

      // NOTE: reauthentication can be switched on or off through the 'TwitterReauthentication.useReauthenticationHack' flag. A reason not to use reauthentication could be that, for secirity reasons, you do not want to have the Twitter API keys (the consumer key and the consumer secret) in the app's source code (which is a necessity for this method to work).
      ...
      // NOTE: if TwitterReauthentication.useReauthenticationHack === 'false' then you can leave the 'consumerKey' and the 'consumerSecretKey' properties of the "TwitterReauthentication" config section empty/blank.

      // If TwitterReauthentication.useReauthenticationHack === 'true' then you will need to configure values for the 'consumerKey' and 'consumerSecretKey' properties, and these values have to be IDENTICAL to the Twitter API keys which you configured in the Firebase dashboard for your Firebase application.
      ...
      function (error) {
        logError(w, "THERE WAS AN ERROR: " + JSON.stringify(error))
        deferred.reject()
      })
    ...
    var loadUnload = function (user, prop, path, load, asArray, orderBy, fbRef, onSuccess, onError, limit) {
      ...
      // IMPORTANT: before loading a new $firebaseObject, call $destroy() on the old $firebaseObject; this is important to ensure that memory is freed and event listeners are cancelled for the $firebaseObject
      if (obj[prop]) {
        obj[prop].$destroy()
        delete obj[prop]
      }
    ...
    $rootScope.$on(loadUserDataSuccess(), function (event, userData) {
      ...
      // this "cannot happen" but I've seen it more than once: user data was loaded but the userRole property EMPTY for some reason ... in this case we set the userRole to "invalid" so that the user will then have to log in again
      if (!userRole) {
    ...
    var setUserRole = function (role) {
      currentLoggedinUser.setUserRole(role)
    }
    ...
    var retrieveProfile = function () {
      ...
      return angular.extend(defaultValues, getUserData())
    }

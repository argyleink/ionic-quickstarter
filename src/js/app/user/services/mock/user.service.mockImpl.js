...
appModule('app.user')
  .service('UserServiceMockImpl', function ($q, $log, $rootScope, User, AppHooks, StorageService) {
    ...
    var init = function () {
      // set logged in user at app startup
      setCurrentUser(userData)
      // comment out the line above and uncomment the next line to require login at startup
      // setCurrentUser(null)    
      // no valid user at application init, forcing login at start up
      return currentLoggedinUser
    }
    ...
    var loadUnload = function (user, prop, load, onSuccess, data) {
      ...
      if (load) {
        obj[prop] = data
        if (onSuccess) $rootScope.$broadcast(onSuccess, data)
      } else {
        delete obj[prop]
      }
    }
